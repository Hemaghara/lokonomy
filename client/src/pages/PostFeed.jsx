        ...
        className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border text-sm transition-all duration-200 outline-none
          ${
            disabled
              ...
              : open
                ? "bg-white/[0.07] border-emerald-500/60 ring-2 ring-emerald-500/10"
...
const PostFeed = () => {
  ...
  useEffect(() => {
    if (feedLocation) {
      setFormData((prev) => ({
        ...prev,
        latitude: feedLocation.lat,
        longitude: feedLocation.lng,
        locationAddress: feedLocation.address,
      }));
    }
  }, [feedLocation]);
  ...
  const feedTypeOptions = feedTypes.map((t) => ({ value: t, label: t }));

  return (
    <div className="min-h-screen bg-[#080e1a] pt-28 pb-24 px-4 relative overflow-hidden">
      <div className="fixed top-0 right-0 w-125 h-125 bg-emerald-500/5 blur-[130px] rounded-full pointer-events-none" />
      <div className="fixed bottom-0 left-0 w-100 h-100 bg-emerald-500/5 blur-[130px] rounded-full pointer-events-none" />
          ...
          >
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div className="sm:col-span-2">
                <MapPicker value={feedLocation} onChange={setFeedLocation} />
              </div>

              <Divider label="Visual Image" />
              ...
              <div className="sm:col-span-2 space-y-3">
                {!formData.image ? (
                  <label className="flex flex-col items-center justify-center gap-3 w-full py-8 rounded-xl border border-dashed border-white/10 cursor-pointer hover:bg-white/3 hover:border-emerald-500/30 transition-all group">
                    ...
                    />
                    <svg
                      className="w-8 h-8 text-white/15 group-hover:text-emerald-500/40 transition-colors"
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                      strokeWidth={1.5}
