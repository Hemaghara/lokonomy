...
const Wishlist = () => {
  ...
  const tabs = [
    ...
    },
    {
      ...
      count: wishlist.businesses.length,
    },
    {
      id: "jobs",
      label: "Jobs",
      icon: <HiOutlineBriefcase />,
      count: wishlist.jobs.length,
    },
  ];

  const card =
    "bg-[#111827] border border-[#1f2a3d] rounded-2xl overflow-hidden group hover:border-violet-500/30 transition-all duration-300";
        ...
        <div className="flex items-center gap-1 bg-[#111827] border border-[#1f2a3d] rounded-2xl p-1 mb-8">
          {tabs.map((tab) => (
            ...
            >
              ...
              {tab.label}
              <span
                className={`ml-1 px-1.5 py-0.5 rounded-md text-[10px] ${activeTab === tab.id ? "bg-white/20" : "bg-white/5"}`}
              >
                {tab.count}
              </span>
            </button>
                ...
                {activeTab === "products" &&
                  wishlist.products.map((p) => (
                    <motion.div layout key={p._id} className={card}>
                      ...
                      <div className="p-5">
                        <h3 className="text-white font-bold text-base mb-1 line-clamp-1 group-hover:text-violet-400 transition-colors">
                          {p.productName}
                        </h3>
                    ...
                    <motion.div layout key={b._id} className={card}>
                      <div className="p-6 flex items-start gap-4">
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className="text-white font-bold text-base truncate group-hover:text-violet-400">
                            {b.businessName}
                          </h3>
                          ...
                          <div className="flex items-center gap-1 text-[10px] text-slate-600">
                            <HiOutlineMapPin className="text-rose-500" />{" "}
                            {b.district}
                          </div>
                        </div>
                        <WishlistButton
                  ...
                  wishlist.jobs.map((j) => (
                    <motion.div layout key={j._id} className={card}>
                      <div className="p-6">
                        ...
                        <h3 className="text-white font-bold text-base mb-1 group-hover:text-violet-400">
                          {j.position}
                        </h3>
