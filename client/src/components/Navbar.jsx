    ...
    <>
      <nav
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled
            ? "bg-dark-bg/80 backdrop-blur-lg border-b border-white/5 py-3"
            : "bg-transparent py-5"
        }`}
      >
            ...
            {user ? (
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-4 border-r border-white/10 pr-6 mr-1">
                  ...
                  <Link
                    to="/my-chats"
                    className="relative text-white/60 hover:text-white transition-colors flex flex-col items-center gap-0.5 group"
                    ...
                    <AnimatePresence>
                      {unreadCount > 0 && (
                        <motion.span
                          ...
                          animate={{ scale: 1 }}
                          exit={{ scale: 0 }}
                          className="absolute -top-2 -right-3 w-4 h-4 bg-violet-600 rounded-full flex items-center justify-center text-[8px] text-white font-bold shadow-lg shadow-violet-600/40"
                        >
                  ...
                  {user ? (
                    <div className="flex flex-col gap-4">
                      ...
                      <Link
                        ...
                        className="flex items-center gap-3 text-text-dim relative"
                      >
                        💬 My Chats
                        {unreadCount > 0 && (
                          <span className="w-5 h-5 bg-violet-600 rounded-full flex items-center justify-center text-[10px] text-white font-bold">
                            {unreadCount}
                          </span>
                        )}
                      </Link>
