...
const AdminCommandPalette = ({ open, onClose }) => {
  ...
  const allResults = [
    ...
    ...(results?.users?.map((u) => ({
      ...
      label: u.name,
      sub: u.email,
      path: `/admin/user/${u._id}`,
      icon: FiUser,
      group: "Users",
      colorClass: COLOR_MAP.users,
    })) || []),
    ...(results?.businesses?.map((b) => ({
      ...b,
      resultType: "business",
      label: b.businessName,
      sub: `${b.mainCategory} · ${b.district}`,
      path: `/admin/business/${b._id}`,
      icon: FiBriefcase,
      group: "Businesses",
      colorClass: COLOR_MAP.businesses,
    })) || []),
    ...(results?.jobs?.map((j) => ({
      ...j,
      resultType: "job",
      label: j.position,
      sub: j.district,
      path: `/admin/jobs/${j._id}`,
      icon: FiFileText,
      group: "Jobs",
      colorClass: COLOR_MAP.jobs,
    })) || []),
    ...(results?.products?.map((p) => ({
      ...
      label: p.productName,
      sub: `₹${p.price}`,
      path: `/admin/marketplace/product/${p._id}`,
      icon: FiPackage,
      group: "Products",
      colorClass: COLOR_MAP.products,
    })) || []),
  ];
    ...
    >
      ...
      >
        <div className="flex items-center gap-3 px-5 py-4 border-b border-white/8">
          <FiSearch
            className={`text-lg shrink-0 transition-colors ${loading ? "text-indigo-400 animate-pulse" : "text-slate-500"}`}
          />
          <input
            ref={inputRef}
            type="text"
          ...
          {Object.entries(groups).map(([group, items]) => {
            ...
            return (
              <div key={group} className="p-2">
                <p className="px-3 pt-2 pb-1 text-[10px] font-black text-slate-600 uppercase tracking-[0.25em]">
                  {group}
                </p>
                {items.map((item) => {
                  ...
                  const colorClass =
                    item.colorClass || "text-indigo-400 bg-indigo-500/10";
                  return (
                    <button
                    ...
                    >
                      ...
                      <FiArrowRight
                        className={`text-sm shrink-0 transition-all ${isSelected ? "text-indigo-400 translate-x-0.5" : "text-slate-700"}`}
                      />
                    </button>
                  );
                })}
        ...
        <div className="flex items-center justify-between px-5 py-3 border-t border-white/5 bg-white/2">
          <div className="flex items-center gap-4 text-[10px] font-bold text-slate-600 uppercase tracking-widest">
            <span className="flex items-center gap-1.5">
              <kbd className="px-1.5 py-0.5 bg-slate-800 rounded text-slate-500 font-black">
                ↑↓
              </kbd>{" "}
              Navigate
            </span>
