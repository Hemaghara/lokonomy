...
const testsDir = path.join(__dirname, '../src/pages/__tests__');
  ...
  files.forEach(file => {
    ...
    if (stat.isDirectory()) {
      ...
      if (!fs.existsSync(testFilePath)) {
        ...
        const template = `import { render, screen } from '${path.relative(path.dirname(testFilePath), path.join(__dirname, '../src/utils/test-utils')).replace(/\\/g, '/')}';
...
describe('${componentName} Page', () => {
  it('renders without crashing', () => {
    ...
    // Add more specific assertions for ${componentName} here
  });
});
`;
...
generateTests(pagesDir, testsDir);
